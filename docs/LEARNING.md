...
### [2026-07-28] 새로고침하면 캐릭터가 화면 중앙에 떴다가 카메라가 튀고, Intro 내용이 늦게 한꺼번에 나타남

- **증상**: 새로고침하면 항공뷰(캐릭터가 화면 중앙)가 잠깐 보이다가 카메라가 정면뷰로 튄다. Intro 내용도 늦게 한꺼번에 나타난다.
- **환경**: React 19, @react-three/fiber v9, Suspense.
- **원인**: `Stations`(비활성 모습)와 `ActiveStationScene`(활성 상세)이 **같은 Suspense 경계** 안에 있었다. 비활성 텍스처가 준비될 때까지 활성 구현이 커밋되지 못해, 첫 화면의 카메라 자세를 잡는 layout effect가 그만큼 늦게 돌았다. 그동안 `CameraRig`는 경계 밖이라 항공뷰로 캐릭터를 따라가고 있었다. Intro 내용이 늦은 것도 같은 경계가 **가장 느린 리소스**를 기다렸기 때문이다.
- **왜 이제야 보였나**: 캐릭터에 텍스처를 입혀보던 임시 변경이 `Character`를 Suspense로 감싸, 로딩 동안 캐릭터를 아예 그리지 않았다. 그래서 "캐릭터가 중앙에 있는 항공뷰" 구간이 눈에 띄지 않았을 뿐 현상은 그대로 있었다. 그 변경을 되돌리자 드러났다.
- **해결**: 경계를 잘게 나눴다. `Stations`와 `ActiveStationScene`을 각각의 경계로, `Stations` 안에서는 스테이션마다 하나씩, `AboutSkillsScene`은 페이지 내용을 자체 경계로 감쌌다.
- **참고**: `src/scene/Experience/Experience.tsx`, `src/scene/Stations/Stations.tsx`
