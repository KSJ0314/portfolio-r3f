...
- **카메라·이동**: 기울어진 항공뷰(Orthographic isometric). **우클릭 홀드로 캐릭터 이동**(거리 무관 고정 속도), 좌클릭은 인터랙션용. 카메라는 캐릭터를 화면 중앙에 두고 따라가는 **캐릭터 팔로우**(현재는 임시 캐릭터, 실제 캐릭터·걷기는 Phase 11). 상세: DECISIONS 003·004.
- **스테이션 = 종이 위의 그림**: 평소엔 2D 그림으로 놓이고, 좌클릭하면 카메라가 포커스하며 그 그림이 제자리에서 **3D로 전환·활성화**돼 상세를 보여준다. 전환 연출은 스테이션마다 다르다(2D 유지 + 카메라 회전 / 2D→3D 애니메이션 / 3D가 펼쳐짐 등). 닫기 → 항공뷰 복귀. 비활성 상태와 활성 연출은 `id → 전용 컴포넌트` 레지스트리로 스테이션별 독립 구현.
- **스테이션은 영역을 갖는다**: 종이 위에 차지하는 사각형이 곧 **클릭 판정 범위이자 근접 판정 기준**이다. 중심점까지의 거리가 아니라 영역 테두리까지의 거리로 근접을 판정하며, 거리 계산은 형태를 아는 스테이션이 맡는다. ([DECISIONS 014])
- **닫기는 근접 이탈·나가기 요소·ESC**: 활성 중에도 평소처럼 이동할 수 있고, 걸어서 영역에서 멀어지는 것이 곧 닫기다. 나가기 UI는 스테이션이 각자 제공한다. 우클릭은 순수하게 이동이다.
- **첫 화면은 Intro 활성 상태**: 사이트에 들어오면 Intro 스테이션이 활성인 채로 시작해 페이지를 정면에서 보여준다. ([DECISIONS 015])
...
- **기본 설명은 Firestore `projects`** 에 저장 → 데이터로 전시대 생성.
- **프로젝트 구역은 스테이션 하나**(`projects`)다. 종이 위에 선 건물이 곧 구역이고, 들어가는 곳은 문 하나뿐이라 문 앞에서 문을 눌러야 활성화된다. ([DECISIONS 031])
- **프로젝트마다 스테이션을 두지 않는다.** 전시대는 건물 안에 놓이고 **Firestore `projects` 개수대로** 만든다. 개수는 고정이 아니라 문서를 더하는 만큼 늘어난다.
- 각 전시대에 **"체험하기" 버튼** → 그 프로젝트의 **메인 기능 인터랙티브 데모**(프로젝트마다 별도 코드 구현)를 **포커스 모드(전용 화면/오버레이)**로 실행.
- 연결: 로컬에 저장한 프로젝트별 `demoKey` ↔ 로컬 데모 레지스트리 매칭([DECISIONS 012] — `demoKey`는 Firestore가 아니라 로컬 상세 데이터에 둔다).

- **건물 안은 별도 라우트(`/projects`)** 다. 문으로 걸어 들어가면 화면이 통째로 갈리고, 오가는 사이는 전환 덮개가 덮는다. 실내는 맵과 세계가 달라 이동·판정·카메라를 나눠 갖는다. ([DECISIONS 034])
- 로비에 들어서면 연단 위 **책**이 전시관 안내이고(오른쪽 페이지에 `projects` 목록), 안쪽 통로로 전시 공간에 들어간다.
...
| **8. 스테이션 상세 구현** ★ | 스테이션 5개(About 3·Projects 1·Guestbook 1)를 8-1→8-2 순으로 하나씩 완전히 끝내고 다음으로 넘어간다. ([DECISIONS 013]) |
| **8-1. 정적 오브젝트** | 공통 임시 박스를 걷어내고 레지스트리에 스테이션별 비활성 상태(종이 위 그림 또는 3D 오브젝트)를 등록, 영역(클릭·근접 판정 범위)과 배치 좌표를 최종값으로 확정 |
| **8-2. 인터랙션 UI** | 레지스트리에 `Scene`/`Overlay`를 등록해 활성화 전환 연출과 Firestore 데이터 렌더링을 구현, 스테이션 성격에 따라 카메라 제어(각도 전환 등)와 나가기 UI까지 직접 구현. 프로젝트는 건물 **안(`/projects` 로비 → 전시 공간)** 까지 이 안에서 만들고 "체험하기" 데모 진입점을, Guestbook은 쓰기 폼·봇 방지·보안 규칙을 이 안에서 함께 구현 |
...
| **10. 폴리싱** | Bloom/네온, 낮/밤 전환 연출(테마 완성), 로딩, 반응형, 성능(Draco·instancing), 접근성, SEO |
