...
- `stand` — 활성화할 때 캐릭터가 서는 자리(월드 좌표). 값은 스테이션 상수가 갖고, 데려가는 일은 공통 함수가 맡는다 — 진입 연출에서 걸어가면 `walkToStand`, 월드맵에서 고르면 `teleportToStand`. 언제 부를지(연출 순서)는 스테이션이 정한다. 등록하지 않으면 진입 이동이 없다. (DECISIONS 023)
