...
- `LobbyScene` (`<Canvas shadows>`) — **원근** 카메라다. 종이 위를 내려다보는 맵과 달리 실내는 깊이가 보여야 방으로 읽힌다. 첫 카메라 자리는 시작 자리에 팔로우 오프셋을 더해 만들고, 노출로 흰 대리석이 타는 것을 화면 안에 들인다. 들어올 때마다 이동·트리거 상태를 처음으로 되돌린다(주소 직접 입력·앞으로가기 대비).
  ...
  - `LobbyCharacter` — 실내 캐릭터(`interior/InteriorCharacter`). 맵과 같은 모델(`scene/CharacterModel`)을 쓰고 크기·밝기만 방이 정한다. 매 프레임 목표점으로 고정 속도 이동하고, 밟는 바닥에 레이캐스트해 높이를 정한다. 막는 것에 겹치면 밀려난다.
...
- `ListViewPage`(`pages/`) — 굽는 동안 안내를 덮고, 다 구우면 `ListBaker`를 걷어 **이미지만 남긴다.** 구운 그림에는 누를 것이 없으므로 링크 자리에 투명한 판을 덮어 살린다. PDF는 누른 뒤가 아니라 굽자마자 미리 묶어두고, 그동안 내려받기 버튼 테두리에 진행률을 표시한다.
  - `ListBaker`(`scene/`) — 화면 밖 캔버스(`<Canvas orthographic>`). 조명·스텐실을 맵과 같게 두되 바탕만 흰색이고 모눈종이는 깔지 않는다. 그린 것을 읽어 가야 하므로 `preserveDrawingBuffer`를 켠다.
    ...
    - `ListBaker.capture` — 카메라만 옮기며 한 장씩 찍는다. **이 컴포넌트는 내용의 Suspense 경계 밖이다** — 안에 두면 무언가 서스펜드할 때 함께 언마운트돼 찍던 차례를 잃는다. 링크 자리는 그린 뒤에 찾는다. 그때라야 월드 행렬이 그 자세로 맞춰져 있다.
