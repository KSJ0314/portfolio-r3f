...
**오른쪽 아래 구석 버튼은 `ui/CornerButton` 하나를 나눠 쓴다.** 자리는 `$slot`(오른쪽에서 몇 번째)으로 계산하므로 크기·간격을 바꿔도 서로 겹치지 않는다. 버튼마다 좌표를 직접 적으면 하나를 손볼 때 나머지가 어긋난다. 오른쪽부터 목록 보기(`ui/ListViewButton`)·에셋 출처(`ui/Credits`)·저장소(`ui/GithubButton`)·크레파스 스튜디오 순이고, 저장소 버튼은 이 포트폴리오의 레포를 새 탭으로 연다.
...
3D를 돌아다니지 않고 주요 화면만 넘겨 보는 페이지다. **인쇄용 화면을 따로 만들지 않고 맵이 쓰는 컴포넌트를 그대로 세워 굽는다** — 콘텐츠가 두 벌이 되면 한쪽만 고쳐 어긋난다. (DECISIONS 048)

- `ListViewPage`(`pages/`) — 굽는 동안 안내를 덮고, 다 구우면 `ListBaker`를 걷어 **이미지만 남긴다.** 구운 그림에는 누를 것이 없으므로 누를 자리에 투명한 판을 덮어 살린다. 주소를 실은 자리는 새 탭으로 열고, 연락처처럼 복사할 값을 실은 자리는 클립보드에 담고 그 위에 잠시 알린다. PDF는 누른 뒤가 아니라 굽자마자 미리 묶어두고, 그동안 내려받기 버튼 테두리에 진행률을 표시한다.
  - `ListBaker`(`scene/`) — 화면 밖 캔버스(`<Canvas orthographic>`). 조명·스텐실을 맵과 같게 두되 바탕만 흰색이고 모눈종이는 깔지 않는다. 그린 것을 읽어 가야 하므로 `preserveDrawingBuffer`를 켠다.
    - `ListBaker.content` — 늘어놓을 화면을 한 줄로 떼어 세운다. 나가기·페이지 넘김은 조작 요소라 두지 않는다. 그림들을 로고 자리로 물리는 신호는 그림들보다 **뒤에** 둬야 각 그림이 구독한 뒤에 켜진다.
...
- `ListBaker.pdf` — 구운 그림을 그대로 넣어 묶는다. 다시 굽지 않으므로 화면과 내려받는 것이 같다. 페이지 크기를 굽는 크기 그대로 둬 여백 없이 꽉 찬다.

굽는 형식은 JPEG다. PNG로 두면 PDF에 넣을 때 장마다 다시 압축하느라 그동안 화면이 통째로 멎는다.

Canvas 밖: `BackButton`(좌상단) · 맵 우측 하단의 진입 버튼(`ui/ListViewButton`).
