...
- `ProjectsGalleryPage` (`<Canvas>`) — 원근 카메라. 칸 수가 정해지기 전(Firestore 읽기 중)에는 방을 세우지 않아 반쯤 지어진 방이 보이지 않는다.
  ...
  - `GalleryPages` — 확대한 칸의 액자 앞에 **불투명한 판**을 액자 크기 그대로 세우고 그 안의 내용만 장 단위로 갈아 끼운다. 판이 표지 사진을 덮으므로 첫 장부터 그 프로젝트의 내용이다. 안쪽은 **가로 1 기준 정규화 좌표**다 — 잰 액자 가로를 그룹 배율로 걸어, 페이지는 칸이 얼마나 큰지 알 필요 없이 비율로만 그린다. **마운트는 확대가 시작될 때, 보이는 것은 확대가 끝난 뒤다** — 도는 도중에 보이면 멀리서 그림만 갈리고, 다 확대된 뒤에 마운트하면 글씨를 배치하는 동안 바탕만 있는 판이 먼저 뜬다. 페이지 목록은 프로젝트 번호로 찾고, 등록 전이면 자리표시 페이지가 나온다. (DECISIONS 040)
    - `GalleryPager` — 하단 점과 좌우 끝 꺾쇠. 점은 지금 보는 장만 채우고 눌러 그 장으로 바로 간다. 첫 장에서 왼쪽을, 마지막 장에서 오른쪽을 두지 않으며 한 장뿐이면 둘 다 없다. 꺾쇠는 그려 둔 SVG다 — 씬 안 3D 글씨는 서브셋 ttf를 직접 읽어 `‹` 같은 글리프가 없을 수 있다.
...
**페이지 정의는 `contents/` 밑에 프로젝트 번호로 둔다.** `index.ts`가 `번호 → 페이지 목록`을 갖고, 등록 전이거나 번호가 없는 프로젝트는 자리표시 페이지를 쓴다(빈 목록을 돌려주면 확대했을 때 아무것도 없는 판만 남는다). **페이지에 들어가는 글은 Firestore가 아니라 여기에 직접 쓴다** — 장마다 필요한 글이 다르고 줄바꿈·강조까지 화면에 맞춰야 해서, 컬렉션 필드로 두면 오히려 손이 더 간다. 페이지 컴포넌트는 그 Firestore 문서를 props로 받지만 쓰지 않아도 된다. 장들이 함께 쓰는 부품은 `contents/shared/`에 둔다 — `PageIcon`(그림 한 장, 판 크기는 세로 기준이고 가로는 그림 비율에서 나온다) · `PageLink`(그 위에 누르는 판을 얹어 새 탭으로 연다) · `IntroPage`(첫 장의 배치 한 벌. 프로젝트 폴더는 글과 링크만 넘기는 껍데기다). 절차·기준은 [PROJECT_PAGES.md](./PROJECT_PAGES.md). (DECISIONS 040)
