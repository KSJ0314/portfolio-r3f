...
- `/list` — 목록 보기(`pages/ListView`). 3D를 돌아다니지 않고 주요 화면만 한 장씩 넘겨 본다. 들어오면 화면 밖 캔버스에서 화면들을 이미지로 굽고, 다 구우면 굽는 자리를 걷어 이미지만 남긴다. 그 이미지를 그대로 PDF로 묶어 내려받는다. (DECISIONS 048)
...
- `ListViewPage` — 굽는 동안 안내를 덮고, 다 구우면 `ListBaker`를 걷어 **이미지만 남긴다.** 구운 그림에는 누를 것이 없으므로 링크 자리에 투명한 판을 덮어 살린다. PDF는 누른 뒤가 아니라 굽자마자 미리 묶어두고, 그동안 내려받기 버튼 테두리에 진행률을 표시한다.
  - `ListBaker` — 화면 밖 캔버스(`<Canvas orthographic>`). 조명·스텐실을 맵과 같게 두되 바탕만 흰색이고 모눈종이는 깔지 않는다. 그린 것을 읽어 가야 하므로 `preserveDrawingBuffer`를 켠다.
...
- `ListView.screens` — 무엇을 몇 장 찍을지. 화면마다 **자기 영역이 16:9에 최대로 들어가는 배율**을 갖는다. 어디에 세울지와 어디를 찍을지를 한자리에서 정한다. 따로 세면 카메라가 엉뚱한 자리를 찍는다. 프로젝트 화면 수는 Firestore 문서 개수와 각 프로젝트의 페이지 수에서 나오므로 프로젝트가 늘면 따라 는다.
- `ListView.pdf` — 구운 그림을 그대로 넣어 묶는다. 다시 굽지 않으므로 화면과 내려받는 것이 같다. 페이지 크기를 굽는 크기 그대로 둬 여백 없이 꽉 찬다.
