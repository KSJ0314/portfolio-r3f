...
- `useLobbyGeometryStore` — **모델에서 잰** 트리거(연단 위 책·북쪽 통로)의 중심과 크기, 그리고 방의 좌우 끝(카메라가 따라갈 범위의 기준). 상수로 박으면 모델을 다시 내보낼 때마다 어긋난다. `useProjectsDoorStore`와 같은 자리다. (DECISIONS 033)
