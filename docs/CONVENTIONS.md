...
커밋 메시지·PR·docs 전부에 적용한다.

- **번역투·구어체를 쓰지 않는다.** 개발 용어는 통용되는 말로 적는다.
  (`한 벌`·`부품` → `컴포넌트`, `장 단위` → `페이지 단위`, `잰` → `측정한`, `갈아 끼운다` → `교체한다`, `값으로 박는다` → `상수로 고정한다`, `손이 더 간다` → `비용이 크다`)
- **UI 동작은 구현 묘사가 아니라 기능 이름으로 쓴다.** `하단 점과 좌우 끝 꺾쇠`가 아니라 `하단 인디케이터와 좌우 버튼으로 페이징 처리`처럼 적는다.
...
- **불필요한 수식을 넣지 않는다.** 작업의 핵심만 남긴다. (`전시 칸 확대 화면과 프로젝트 상세 페이지 구현` → `프로젝트 상세 페이지 구현`)
...
- **문장이 끝난 자리(마침표 뒤)에서 줄을 바꾼다.** 한 불릿이 두 문장 이상이면 마침표마다 줄을 나누고 두 칸 들여쓰기로 이어 쓴다. 커밋 메시지와 같은 기준이다.

아래는 반복해서 어긋난 것들이다. 예시는 캐릭터 모델 적용 PR에서 실제로 고친 문장이다.

- **개요는 무엇을 왜 하는지까지 쓴다.** 단계 이름만 적지 않는다.
  - 나쁜 예: `Phase 11 캐릭터 단계다.`
  - 좋은 예: `Phase 1부터 이동과 카메라를 검증하려고 사용한 임시 박스를 실제 캐릭터 모델로 교체한다.`
- **한 문장으로 쓸 수 있으면 쪼개지 않는다.** 이유는 문장 안에 넣고 `~한다. ~때문이다`로 이어 붙이지 않는다.
  - 나쁜 예: `모델과 애니메이션은 맵과 실내가 CharacterModel 하나를 공유하고, 이동과 회전은 각 방이 맡는다. 라우트가 갈려 이동 규칙이 다르기 때문이다.`
  - 좋은 예: `CharacterModel은 모델과 애니메이션만 담당하고 이동과 회전은 각 씬의 캐릭터 컴포넌트가 처리한다.`
- **구어체를 개발 용어로 바꾼다.**
  (`고른다` → `선택한다`, `맡는다` → `담당한다`, `미리 받는다` → `미리 로드한다`, `태운다` → `적용한다`, `싣는다` → `추가한다`, `끊는다` → `중단한다`, `잰다` → `측정한다`)
  - 나쁜 예: `걷기와 뛰기가 한 파일에 들어 있어 클립 이름으로 고른다.`
  - 좋은 예: `걷기와 뛰기가 단일 파일에 포함되어 클립 이름으로 선택한다.`
- **불릿은 각각 독립이다.** 앞 항목에 기대어 주어를 생략하지 않는다.
  - 나쁜 예: `서 있는 동작이 없어 멈추면 재생을 끊고 모델의 기본 자세로 선다.`
  - 좋은 예: `정지 상태 애니메이션이 없으므로 이동이 종료되면 재생을 중단해 모델의 기본 자세를 표시한다.`
- **버그 증상은 과거형(`~하는 버그가 있었다`·`~한 현상이 있었다`)으로, 조치는 현재형으로 쓴다.** 고친 것을 현재 남아 있는 것처럼 쓰지 않는다.
  - 나쁜 예: `공통층이 근접 판정에 반경 2를 추가해 건물 측면에서도 프로젝트 구역이 활성화된다.`
  - 좋은 예: `공통층이 근접 판정에 반경 2를 추가해 건물 측면에서도 프로젝트 구역이 활성화되는 버그가 있었다.`
- **증상과 원인은 한 문장에 담고 조치를 다음 문장에 쓴다.** 증상과 원인을 문장으로 가르지 않는다.
  - 나쁜 예: `건물 측면에서도 프로젝트 구역이 활성화되는 버그가 있었다. 공통층이 근접 판정에 반경 2를 추가한 것이 원인이다.`
  - 좋은 예: `공통층이 근접 판정에 반경 2를 추가해 건물 측면에서도 프로젝트 구역이 활성화되는 버그가 있었다.`
- **`## 검증`은 타입체크·lint 한 줄과 `로컬 환경에서 테스트` 한 줄을 두고, 확인한 항목은 그 아래 하위 목록으로 `~것 확인` 꼴로 쓴다.** 줄마다 `로컬 환경에서`를 반복하지 않는다.

```
- 타입체크(`tsc -b`)·lint 통과
- 로컬 환경에서 테스트
  - 맵과 실내에서 캐릭터가 진행 방향으로 돌며 이동하는 것 확인
  - 스테이션을 열 때 캐릭터가 그 대상 쪽으로 도는 것 확인
```
