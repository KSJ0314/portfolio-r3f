...
> 이 문서는 프로젝트의 현재 진행 상황을 추적합니다. 마지막 업데이트: **2026-08-03**
> 전체 계획은 [PLAN.md](./PLAN.md) 참고.

## 현재 단계

**Phase 8 — 스테이션 상세 구현 진행 중** (`about-intro`·`about-skills` 완료, 다음 스테이션으로)
...
1. `about-career` 정적 오브젝트 — 종이 위에 놓일 모습
2. `about-career` 인터랙션 UI — 활성 연출 + Firestore `experiences`·`education`·`spec` 렌더링
3. 나머지 스테이션을 위 체크리스트 순서대로 하나씩
