 ...
 * 스테이션이 열려 있는 동안에는 마운트되지 않으므로(App) 근접·선택 강조는 두지 않는다.
 * 누르면 맵 전체를 보는 월드맵이 열린다.
 */
const VIEW_RADIUS = 16
/** 가장자리 페이드 폭(유닛). */
const FADE = 3

export function Minimap({ onOpen }: MinimapProps) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      for (const station of STATIONS) {
        ...
        if (!g || !station.position) continue
  ...
  return (
    <MinimapPanel type="button" onClick={onOpen} title="월드맵 열기" aria-label="월드맵 열기">
      <MinimapSvg viewBox={`${-VIEW_RADIUS} ${-VIEW_RADIUS} ${2 * VIEW_RADIUS} ${2 * VIEW_RADIUS}`}>
        ...
        {/* 스테이션 마커 — 위치/표시는 rAF가 imperative로 갱신한다. 자리가 정해진 것만 그린다. */}
        {STATIONS.map((station) => {
          ...
          return (
            <g
              ...
              style={{ opacity: 0 }}
            >
              <circle cx={0} cy={0} r={1.2} fill={color} stroke="#ffffff" strokeWidth={0.4} />
              <text
                x={0}
                y={-3.2}
                textAnchor="middle"
        ...
        {/* 플레이어(항상 중앙) */}
        <circle cx={0} cy={0} r={1.4} fill={theme.colors.accent} stroke="#ffffff" strokeWidth={0.5} />
      </MinimapSvg>
    </MinimapPanel>
  )
}
