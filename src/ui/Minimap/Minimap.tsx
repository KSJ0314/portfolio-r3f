...
/**
 * 게임식 원형 미니맵. 플레이어를 중앙에 고정하고, 주변 VIEW_RADIUS(월드 유닛) 안의
 * 스테이션만 표시한다(맵 전체가 아니라 근처 상호작용 요소만).
 *
 * 화면의 아이소메트릭 뷰는 지면상 돌아가 보이므로, 미니맵도 같은 각도로 회전시켜 실제 화면
 * 방향과 일치시킨다. 각도는 카메라에서 유도한 store의 viewAngle을 사용(하드코딩 아님).
 * 회전은 각 마커의 상대 좌표에 직접 적용(거리 보존)하고, 라벨 텍스트는 회전 그룹 밖
 * (translate만)에 두어 똑바로 선 채로 유지한다.
 *
 * 플레이어 위치는 매 프레임 좌표만 바뀌므로 리렌더 없이 rAF로 각 마커의 transform·opacity만
 * 직접 갱신한다. 근접/선택 강조는 자주 안 바뀌므로 store 구독(리렌더)으로 처리.
 */
