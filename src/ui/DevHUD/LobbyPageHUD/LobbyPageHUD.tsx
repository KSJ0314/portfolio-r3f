...
export function LobbyPageHUD() {
  ...
  const [values, set] = useControls('로비', () => ({
    ...
    '책 보기': folder(
      {
        ...
        focusSeconds: {
          ...
          step: 0.1,
          label: '도는 시간',
          hint: hint('트리거를 보러 도는 데 걸리는 시간(초). 닫힐 때도 같다.', T.focusSeconds),
        },
      },
      { collapsed: true },
    ),
    '액자 보기': folder(
      {
        artworkDistance: {
          value: T.artworkDistance,
          min: 0.5,
          max: 12,
          step: 0.05,
          label: '거리',
          hint: hint(
            '액자와 카메라 사이의 거리. 좌우 액자가 함께 쓰고 방향만 반대다.',
            T.artworkDistance,
          ),
        },
        artworkY: {
          ...
          step: 0.05,
          label: '높이 차',
          hint: hint('액자 중심과의 높이 차. 0이면 액자와 같은 높이에서 본다.', T.artworkY),
        },
        artworkZ: {
          value: T.artworkZ,
          min: -4,
          ...
          step: 0.05,
          label: '앞뒤',
          hint: hint('액자 중심에서의 앞뒤 거리. 0이면 액자 한가운데를 마주 본다.', T.artworkZ),
        },
        artworkShiftX: {
          value: T.artworkShiftX,
          min: -1,
          max: 1,
          step: 0.01,
          label: '구도 가로',
          hint: hint('0이면 액자가 화면 한가운데. 양수면 오른쪽으로 간다.', T.artworkShiftX),
        },
        artworkShiftY: {
          value: T.artworkShiftY,
          min: -1,
          max: 1,
          step: 0.01,
          label: '구도 세로',
          hint: hint('0이면 액자가 화면 한가운데. 음수면 아래로 내려간다.', T.artworkShiftY),
        },
      },
      { collapsed: true },
    ),
    ...
    '로비',
    {
        ...
        const json = JSON.stringify(
          {
            ...
            artworkFocus: {
              distance: trigger.artworkDistance,
              y: trigger.artworkY,
              z: trigger.artworkZ,
              shift: { x: trigger.artworkShiftX, y: trigger.artworkShiftY },
            },
      ...
      '기본값으로': button(() =>
        set({
          ...
          artworkDistance: T.artworkDistance,
          artworkY: T.artworkY,
          artworkZ: T.artworkZ,
          artworkShiftX: T.artworkShiftX,
          artworkShiftY: T.artworkShiftY,
  ...
  useEffect(() => {
    setTrigger({
      ...
      artworkDistance: values.artworkDistance,
      artworkY: values.artworkY,
      artworkZ: values.artworkZ,
      artworkShiftX: values.artworkShiftX,
      artworkShiftY: values.artworkShiftY,
