...
import { INK, careerColumnLeft } from '../AboutCareer.constants'
import { CAREER_COLUMN_TITLES, CAREER_TITLES_Y } from './CareerTitles.constants'

/**
 * 활성 상태에서 칸마다 붙는 손글씨 제목 — 교육 · 수상내역 · 자격증.
 *
...
export function CareerTitles() {
  ...
  const y = area.height / 2 - padding.y - logo.top + title.offsetY
  ...
  return (
    ...
    >
      {CAREER_COLUMN_TITLES.map((text, index) => {
        ...
        const x =
          careerColumnLeft(index, area.width, padding.x) +
          logo.left +
          (widths[index] ?? 0) +
          title.gap
