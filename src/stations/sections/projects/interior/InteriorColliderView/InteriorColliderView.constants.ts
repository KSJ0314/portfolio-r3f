import type { InteriorColliderKind } from './InteriorColliderView.types'

/** 역할별 색. 서로 확실히 갈리는 원색으로 둔다 — 실내가 어두워 옅은 색은 묻힌다. */
export const INTERIOR_COLLIDER_COLOR: Record<InteriorColliderKind, string> = {
  walkable: '#3ddc84',
  blocker: '#ff4d4d',
  overhead: '#4d8dff',
  trigger: '#ffd24d',
 ...
 */
export const INTERIOR_COLLIDER_EXTEND_STEP = 0.3

/**
 * 콜라이더를 앞으로 당기는 정도. 벽·바닥 면과 겹쳐 있어 그대로 두면 z-파이팅으로 얼룩진다.
 * 음수라 깊이가 카메라 쪽으로 밀려 콜라이더가 이긴다.
 */
export const INTERIOR_COLLIDER_DEPTH_BIAS = -2
